 ...
 * @package    EZ Realty
 */
class ezrealtyModelCountrys extends JModelList                   
{     
...
;
   ...
   */ 
	...
	function getCountryFiltList() {	
		...
		. "\n INNER JOIN #__ezrealty AS b ON c.id = b.cnid "
        . "\n WHERE c.published = 1"
        . "\n GROUP BY c.id"
        . "\n $orderby ";
     ...
     */
    function getCountrysById() {

        // Load the data
        if (empty( $this->_data )) {
            $query = ' SELECT * FROM #__ezrealty_country '.
                    '  WHERE id = '.$this->_id;
            $this->_db->setQuery( $query );
	...
	function getPagination(){
		...
		if (empty($this->_pagination))	{
			...
			if (!isset($this->_total)) $this->getCountrys(); 
	...
	function publish () {
        ...
        $pub = false;
		if ($task == 'publish'){
			$sql = "update #__ezrealty_country set published='1' where id in ('".implode("','", $cids)."')";
            $pub = true;
		} else {
			$sql = "update #__ezrealty_country set published='0' where id in ('".implode("','", $cids)."')";
		}
    ...
    function docheckin () {
        ...
        $thecheck = false;
        if ($task == 'checkin') {
            $sql = "update #__ezrealty_country set checked_out='0' where id in ('".implode("','", $cids)."')";
            $thecheck = true;
        }
