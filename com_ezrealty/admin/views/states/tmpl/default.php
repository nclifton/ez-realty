...
<?php endif;?>
	...
	<div id="filter-bar" class="btn-toolbar">
		...
		<div class="btn-group pull-left">
			<button class="btn hasTooltip" type="submit" title="<?php echo JText::_('JSEARCH_FILTER_SUBMIT'); ?>" onclick="this.form.submit();"><i class="icon-search"></i></button>
				<button class="btn hasTooltip" type="button" title="<?php echo JText::_('EZREALTY_RESET_BTN'); ?>" onclick="document.getElementById('search').value='';<?php if ($this->ezrparams->get( 'er_country' ) == 1) { ?>this.form.getElementById('filter_country').value='0';<?php } ?>this.form.submit();"><i class="icon-remove"></i></button>
		</div>
	...
	<table class="table table-striped" id="articleList">
		...
		for ($i=0, $n=count( $items ); $i < $n; $i++) {
			...
			$row->id 	= $row->id;
			$link 		= 'index.php?option=com_ezrealty&controller=states&task=edit&hidemainmenu=1&cid='. $row->id;
		...
		?>
			<tr class="<?php echo "row$k"; ?>">
				...
				<?php if ($this->ezrparams->get( 'er_country' ) == 1) { ?>
					<td class="nowrap small has-context">(<?php echo $row->countryname; ?>)</td>
				<?php } ?>
