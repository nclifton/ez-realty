...
if ($this->params->get('show_print_results')){
	$printlink	= JRoute::_('index.php?option=com_ezrealty&view=state&id='.$id.'&format=pdf', false);
}
...
?>
	...
	<?php if ( $this->params->get( 'show_keyword_select' ) ) { ?>
		<div class="row-fluid">
			<div class="span12">
				<input type="text" name="filter_a4search" id="filter_a4search" placeholder="<?php echo JText::_('COM_EZREALTY_KEYWORD'); ?>" value="<?php echo htmlspecialchars($this->lists['filter_a4search']);?>" class="<?php echo $er_fieldsize;?>" onchange="document.adminForm.submit();" />
			</div>
		</div>
	<?php } ?>
	...
	if ( file_exists(JPATH_SITE . '/administrator/components/com_realtybookings/realtybookings.php') ) {
		if ( $this->params->get( 'use_realtybookings' ) && $this->params->get( 'show_dates_select' ) ) { ?>
			<div class="row-fluid">
				<div class="span6">
					<label class="filter-hide-lbl" for="filter_a4begin"><?php echo JText::_('EZREALTY_BEGIN'); ?></label>
					<?php echo JHtml::_('calendar', $this->lists['filter_a4begin'], 'filter_a4begin', 'filter_a4begin', '%Y-%m-%d' , array('size'=>10, 'onchange'=>"this.form.fireEvent('submit');this.form.submit()"));?>
				</div>
				<div class="span6">
					<label class="filter-hide-lbl" for="filter_a4end"><?php echo JText::_('EZREALTY_END'); ?></label>
					<?php echo JHtml::_('calendar', $this->lists['filter_a4end'], 'filter_a4end', 'filter_a4end', '%Y-%m-%d' , array('size'=>10, 'onchange'=>"this.form.fireEvent('submit');this.form.submit()"));?>
				</div>
			</div>
		<?php }
	} ?>
	...
	$this->params->get( 'show_custom1_select' ) == 0 ) { } else { ?>
			...
			<?php if ( $this->params->get( 'show_categories_select' ) ) { ?>document.getElementById('filter_a4cid').value='0';<?php } ?>
			<?php if ( $this->params->get( 'show_transtype_select' ) ) { ?>document.getElementById('filter_a4type').value='0';<?php } ?>
			<?php if ( $this->params->get( 'show_marketstatus_select' ) ) { ?>document.getElementById('filter_a4sold').value='0';<?php } ?>
			<?php if ( $this->params->get( 'show_sellers_select' ) ) { ?>document.getElementById('filter_a4seller').value='0';<?php } ?>
			<?php if ( $this->params->get( 'show_suburbs_select' ) ) { ?>document.getElementById('filter_a4locality').value='0';<?php } ?>
			<?php if ( $this->params->get( 'show_minmaxprice_select' ) ) { ?>document.getElementById('filter_a4minprice').value='';document.getElementById('filter_a4maxprice').value='';<?php } ?>
			<?php if ( $this->params->get( 'show_minbedsbaths_select' ) ) { ?>document.getElementById('filter_a4minbed').value='0';document.getElementById('filter_a4minbaths').value='';<?php } ?>
			<?php if ( $this->params->get( 'show_minmaxarea_select' ) ) { ?>document.getElementById('filter_a4minarea').value=''; document.getElementById('filter_a4maxarea').value='';<?php } ?>
			<?php if ( $this->params->get( 'show_keyword_select' ) ) { ?>document.getElementById('filter_a4search').value='';<?php } ?>
			<?php if ( file_exists(JPATH_SITE . '/administrator/components/com_realtybookings/realtybookings.php') ) {
				if ( $this->params->get( 'use_realtybookings' ) && $this->params->get( 'show_dates_select' ) ) { ?>document.getElementById('filter_a4begin').value=''; document.getElementById('filter_a4end').value='';<?php } ?>
			<?php } ?>
			<?php if ( $this->params->get( 'show_custom1_select' ) ) { ?>document.getElementById('filter_a4custom1').value='';<?php } ?>
			<?php if ( $this->params->get( 'show_suburbs_select' ) && $this->params->get( 'show_surrounding_select' )){ ?>
				document.getElementById('filter_a4radius').value='NULL';
			<?php } ?>

			<?php if ( $this->params->get( 'show_minmaxland_select' ) ) { ?>
				document.getElementById('filter_a4minland').value='';
				document.getElementById('filter_a4maxland').value='';
				document.getElementById('filter_a4landtype').value=0;
			<?php } ?>
