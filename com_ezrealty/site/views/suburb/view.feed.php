...
class EzrealtysViewSuburb extends JViewLegacy
{
	function display($tpl = null)
	{
		...
		$document->link = JRoute::_(EzrealtyHelperRoute::getSuburbRoute(JRequest::getVar('id',null, '', 'int')));
		...
		foreach ( $items as $item )
		{
			...
			$subname = $this->escape($suburb->name);
			$subname = html_entity_decode($subname, ENT_COMPAT, 'UTF-8');

			// url link to the item
			$link = JRoute::_(EzrealtyHelperRoute::getEzrealtyRoute($item->slug, '', $item->subslug, '' ));
