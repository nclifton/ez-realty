...
class ezrealtyViewCountrys extends JViewLegacy {
    ...
    function display($tpl = null) {
		...
		JToolBarHelper::title( JText::_('COM_EZREALTY_PAGETITLE_COUNTRIES'), 'langmanager.png' );		
		...
		JHtmlSidebar::addEntry(JText::_('COM_EZREALTY_SUBMENU_COUNTRYS'), 'index.php?option=com_ezrealty&controller=countrys', true);
    ...
    function edit($tpl = null) {
		...
		$countrys	=& $this->get('CountrysById');
		$isNew		= ($countrys->id < 1);

		$text = $isNew ? JText::_( 'COM_EZREALTY_PAGETITLE_ADD' ) : JText::_( 'COM_EZREALTY_PAGETITLE_EDIT' );
		JToolBarHelper::title(   JText::_( 'COM_EZREALTY_PAGETITLE_COUNTRIES' ).': <small><small>[ ' . $text.' ]</small></small>', 'langmanager.png' );
		...
		$lists['published'] 	= JHTML::_('select.genericlist',   $publishit, 'published', 'class="input-medium" size="1"', 'value', 'text', $countrys->published );
		...
		$lists['language'] 	= JHTML::_('select.genericlist',   $ltypelist, 'language', 'class="input-medium" size="1"', 'value', 'text', $countrys->language );


        # Build Ordering list

        $orders =& $this->get('Ordering');   
        $lists['orderlist'] = JHTML::_('select.genericlist', $orders, 'ordering', 'class="input-medium" size="1"', 'value', 'text', intval( $countrys->ordering ) );        
