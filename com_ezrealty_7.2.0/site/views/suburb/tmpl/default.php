...
<div id="system" class="container-fluid">
	...
	<div class="row-fluid">
		<div class="span12">
			...
			<?php } ?>
			<?php if($this->params->get('show_suburb_title')) { ?>
				<h2>
					<span>
						<?php echo JHtml::_('content.prepare', $this->suburb->ezcity); ?>
					</span>
				</h2>
			<?php } ?>
			<?php if ($this->params->def('show_suburb_description') && $this->suburb->ezcity_desc || $this->params->def('show_suburb_image') && $this->suburb->image) { ?>
				<div class="category-desc">
					<?php if ($this->params->get('show_suburb_image') && $this->suburb->image) { ?>
						<img src="<?php echo $this->baseurl .'/'. 'images/'. $this->suburb->image;?>" align="<?php echo $this->params->get('catimg_align');?>" style="margin-bottom: 10px; margin-left: 10px; margin-right: 10px" alt="<?php echo $this->suburb->ezcity;?>" />
					<?php } ?>
					<?php if ($this->params->get('show_suburb_description') && $this->suburb->ezcity_desc) { ?>
						<?php echo JHtml::_('content.prepare', $this->suburb->ezcity_desc); ?>
					<?php } ?>
				</div>
				<div class="gen-1"></div>
			<?php } ?>
