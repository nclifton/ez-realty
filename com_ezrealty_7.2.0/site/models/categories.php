 ...
 *
 */
class EzrealtysModelCategories extends JModelLegacy
{
	/**
	...
	function _buildQuery()
	{
		...
		if (JFactory::getApplication()->getLanguageFilter()) {
			$where[] = 'cc.language in (' . $db->Quote(JFactory::getLanguage()->getTag()) . ',' . $db->Quote('*') . ')';
		}
		...
		//Query to retrieve all categories that belong under the components section and that are published and have an image.
		$query = 'SELECT cc.*, g.title AS grouptitle, '
		. ' CASE WHEN CHAR_LENGTH(cc.alias) THEN CONCAT_WS(\':\', cc.id, cc.alias) ELSE cc.id END as slug'
		. ' FROM #__ezrealty_catg AS cc'
		. ' LEFT JOIN #__viewlevels AS g ON g.id = cc.access'
		. $where
		. $whichorder
		;
	...
	/**
	 * Method to get item data for the categories
	 *
	 */
	...
	function _buildFeatured()
	{
		...
		$howmany = ' LIMIT '. $params->get('cats_spotlight_number');
